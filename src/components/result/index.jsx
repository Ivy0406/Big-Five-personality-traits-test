...
const Result = () => {
  return (
    <main className="w-full h-screen flex flex-col">
      <TraitHero traitType={mockData.targetCategory.en} className="pt-6 flex-1">
        <div className="w-full max-w-355 mx-auto flex flex-col justify-between h-full pb-12.25">
          <div className="w-full  flex justify-between">
            <TraitNames traitNames={mockData.traitNames} />
            <Navbar options={mockData.allCategoriesZh} />
          </div>
          <Category
            title={mockData.targetCategory}
            description={mockData.targetCategoryDescription}
          />
        </div>
      </TraitHero>
      <TraitScore
        scoreLevel={mockData.scoreLevel}
        description={mockData.resultDescription.high}
        className="flex-1"
      />
    </main>
  );
